    ...
    return (
        <main className="flex-1 flex flex-col items-center justify-center min-h-screen p-6 relative z-10">
            ...
            <div className="w-full max-w-md glass-card p-10 flex flex-col items-center text-center relative z-10 shadow-2xl">
                ...
                <h1 className="text-3xl font-extrabold mb-2 text-white">Reset Password</h1>
                ...
                <form onSubmit={handleForgotPassword} className="w-full flex flex-col gap-4">
                    ...
                    >
                        {loading ? <Loader2 className="w-5 h-5 animate-spin text-black" /> : "Send Reset Link"}
                    </button>
                </form>

                <div className="mt-8 text-sm text-[var(--color-neon-muted)]">
                    Remember your password? <Link href="/login" className="text-[var(--color-neon-primary)] hover:underline">Log in</Link>
                </div>
            </div>
        </main>
    );
